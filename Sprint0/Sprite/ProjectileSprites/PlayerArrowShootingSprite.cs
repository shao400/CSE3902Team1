...
{
    //Zina
    public class PlayerArrowShootingSprite : ISprite
    {
        private static SpriteBatch mySpriteBatch;
        private static Texture2D myTexture;
        Color myColor = Color.CornflowerBlue;
        Rectangle destinationRec;
        Rectangle sourceRec = new Rectangle(210, 200, 16, IntegerHolder.Five);
        SpriteEffects myEffect;
        float myRotation;

        public PlayerArrowShootingSprite(SpriteEffects effect, float rotation)
        {
            myEffect = effect;
            myRotation = rotation;
